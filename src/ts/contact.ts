...
const isNotEmpty = (val: string): boolean => val.trim().length > 0;
...
const initContactForm = (): void => {
  ...
  const topicInput = document.getElementById('contact-topic') as HTMLInputElement;
  ...
  const hideFormErrorIfValid = (): void => {
    if (formError.style.display !== 'block') return;
  ...
  form.addEventListener('submit', (e) => {
    ...
    if (!isNameValid || !isEmailValid || !isTopicValid || !isMessageValid) {
      ...
      else if (!isTopicValid) topicInput.focus();
