...
const pickProducts = (
  ...
  const remaining = products.filter(
    (product) => !matching.some((item) => item.id === product.id),
  );
...
class Slider {
  ...
  constructor(private readonly root: HTMLElement) {
    ...
    this.slides = Array.from(
      root.querySelectorAll<HTMLElement>("[data-slider-slide]"),
    );
    this.nextBtns = Array.from(
      root.querySelectorAll<HTMLButtonElement>("[data-slider-next]"),
    );
    this.prevBtns = Array.from(
  ...
  private updateBtns(): void {
    ...
    const atEnd =
      this.currentIndex >= this.totalSlides - this.getSlidesToShow();

    this.prevBtns.forEach((btn) =>
      btn.classList.toggle("slider-btn--disabled", atStart),
    );
    this.nextBtns.forEach((btn) =>
      btn.classList.toggle("slider-btn--disabled", atEnd),
    );
  }
  ...
  private init(): void {
    this.nextBtns.forEach((btn) =>
      btn.addEventListener("click", () => this.animateTo(this.targetIndex + 1)),
    );
    this.prevBtns.forEach((btn) =>
      btn.addEventListener("click", () => this.animateTo(this.targetIndex - 1)),
    );
    ...
    this.track.addEventListener(
      "touchstart",
      (e) => this.onDragStart(e.touches[0].clientX),
      { passive: true },
    );
    this.track.addEventListener(
      "touchmove",
      (e) => this.onDragMove(e.touches[0].clientX),
      { passive: true },
    );
